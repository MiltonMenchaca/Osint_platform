...
const EntitiesList: React.FC<EntitiesListProps> = ({
  ...
  const loadEntities = async () => {
    try {
      ...
      const response = await apiService.getEntities(params, investigationId);
  ...
  const handleEntityDelete = async (entityId: string) => {
    try {
      const res = await apiService.deleteEntity(entityId, investigationId);
      if (!res.success) {
        throw new Error(res.message || 'Error al eliminar la entidad');
      }
