...
const RelationshipManager: React.FC<RelationshipManagerProps> = ({
  ...
  const loadAvailableEntities = async () => {
    try {
      const response = await apiService.getEntities(undefined, investigationId);
  ...
  const handleAddRelationship = async () => {
    ...
    try {
      ...
      if (!investigationId) {
        throw new Error('Investigación requerida para crear relación');
      }
  ...
  const handleDeleteRelationship = async (relationshipId: string) => {
    const result = await Swal.fire({
      title: '¿Estás seguro?',
      text: "¿Estás seguro de que quieres eliminar esta relación?",
      icon: 'warning',
    ...
    try {
      ...
      if (!investigationId) {
        throw new Error('Investigación requerida para eliminar relación');
      }

      const response = await apiService.deleteEntityRelationship(relationshipId, investigationId);
      if (!response.success) {
